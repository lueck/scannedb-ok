...
-- | Collect the glyphs of a page into lines.
--
-- 1) We construct a window, that spans the pagewidth but is not heigh
-- and that steps the height of the page from the bottom to the
-- top. It thereby counts the glyphs at each height. The windows
-- overlap.
--
-- 2) The resulting list of glyphs in each height is then split where
-- the count of glyphs is zero. The result is a list of lists, where
...
findLinesWindow :: (Eq g, Glyph g) => Int -> Int -> [g] -> [[g]]
findLinesWindow linesCount stepsPerLine glyphs =
  ...
  splitWhen ((==0) . length) glyphsInWindow -- split by empty windows
  ...
  where
    ...
    top = top_ + 1.0
    bottom = bottom_ - window - 1.0 -- bottom - window - 1
    bottom_ = glyphsBottom glyphs
